...
# MARU FIX: 실제 19개 공공데이터 API 호출 옵션
live_19api_enabled = st.sidebar.checkbox("✅ 실시간 19API 호출 ON", value=True)
live_rc_date = st.sidebar.text_input("실시간 분석 날짜", value=pd.Timestamp.today().strftime("%Y%m%d"))
live_race_no = st.sidebar.number_input("실시간 경주번호", min_value=1, max_value=20, value=1, step=1)
api_timeout_sec = st.sidebar.slider("API 응답 대기", 3, 15, 6)
...
# ===== MARU FIX: 19개 공공데이터 API 실제 호출/진단 엔진 =====
def _maru_add_params(url, params):
    try:
...
def _maru_json_to_df(obj):
    ...
    for path in paths:
        ...
        if ok:
            ...
            if isinstance(cur, list):
                try:
                    return pd.json_normalize(cur)
                except Exception:
                    return pd.DataFrame(cur)
    # 마지막 방어: dict/list가 묻혀 있으면 찾아냄
    def walk(x):
    ...
    if isinstance(got, list):
        try:
            return pd.json_normalize(got)
        except Exception:
            return pd.DataFrame(got)
    return pd.DataFrame()


def _maru_xml_to_df(text):
    try:
...
def _maru_fetch_one_19api(api_name, base_url, rc_date, race_no, meet_name):
    key = str(api_key or "").strip()
    if not key:
        return pd.DataFrame(), "API Key 없음", ""
    base_url = str(base_url or "").strip()
    if not base_url:
        return pd.DataFrame(), "URL 없음", ""
    meet_code = {"서울": "1", "제주": "2", "부산경남": "3", "부경": "3"}.get(str(meet_name), str(meet_name))
    common = {"serviceKey": key, "pageNo": 1, "numOfRows": 100, "rcDate": str(rc_date), "rcNo": int(race_no), "meet": meet_code}
    variants = [
        _maru_add_params(base_url, {**common, "resultType": "json"}),
        _maru_add_params(base_url, {**common, "_type": "json"}),
        _maru_add_params(base_url, {"serviceKey": key, "pageNo": 1, "numOfRows": 100, "raceDate": str(rc_date), "raceNo": int(race_no), "meetCd": meet_code, "resultType": "json"}),
    ]
    last_msg = ""
    last_url = ""
    for url in variants:
        last_url = url
        try:
            r = requests.get(url, timeout=int(globals().get("api_timeout_sec", 6)))
            if r.status_code != 200:
                last_msg = f"HTTP {r.status_code}"
                continue
            text = (r.text or "").strip()
            if any(bad in text for bad in ["SERVICE_KEY_IS_NOT_REGISTERED", "SERVICE_ACCESS_DENIED", "INVALID_REQUEST_PARAMETER", "LIMITED_NUMBER_OF_SERVICE_REQUESTS_EXCEEDS_ERROR"]):
                last_msg = text[:180]
                continue
            ctype = r.headers.get("content-type", "").lower()
            if text.startswith("{") or text.startswith("[") or "json" in ctype:
                try:
                    df = _maru_json_to_df(r.json())
                except Exception:
                    df = pd.DataFrame()
            else:
                df = _maru_xml_to_df(text)
...
def _maru_mask_url(url):
    s = str(url or "")
    k = str(api_key or "")
    if k and k in s:
        s = s.replace(k, k[:5] + "****" + k[-4:])
    return s


def _maru_build_horse_df_from_apis(data_map, course_name, rc_date, race_no):
    rows = {}
    # 출전표/체중/레이팅/배당 등에서 마번/마명 후보 모으기
    for api_name, df in data_map.items():
        if df is None or df.empty:
            continue
        no_col = _maru_find_col(df, ["chulNo", "chul_no", "horseNo", "hrNo", "마번", "출전번호", "출전마번"])
        if no_col is None:
            continue
        name_col = _maru_find_col(df, ["hrName", "horseName", "경주마명", "마명", "경주마"])
        for _, r in df.iterrows():
            ...
            if n < 1 or n > 30:
                continue
            if n not in rows:
                rows[n] = {c: np.nan for c in COLUMNS}
                rows[n].update({"경주일": pd.Timestamp.today().normalize(), "경마장": course_name, "경주번호": int(race_no), "경주마": f"{n}번", "마번": n, "줄": f"{n}째줄", "주로상태": "실시간API"})
            if name_col is not None and str(r.get(name_col, "")).strip() not in ["", "nan", "None"]:
                rows[n]["경주마"] = str(r.get(name_col)).strip()
    if not rows:
        return pd.DataFrame()
    out = pd.DataFrame(list(rows.values()))

    def map_value(api_key_name, target, candidates):
        df = data_map.get(api_key_name, pd.DataFrame())
        if df is None or df.empty:
            return
        no_col = _maru_find_col(df, ["chulNo", "chul_no", "horseNo", "hrNo", "마번", "출전번호", "출전마번"])
        val_col = _maru_find_col(df, candidates)
        if no_col is None or val_col is None:
            return
        tmp = df[[no_col, val_col]].copy()
        tmp[no_col] = pd.to_numeric(tmp[no_col], errors="coerce")
        mp = dict(zip(tmp[no_col].dropna().astype(int), tmp.loc[tmp[no_col].notna(), val_col]))
        out[target] = out["마번"].map(mp).combine_first(out.get(target))

    map_value("body_url", "현재체중", ["wg", "weight", "체중", "hrWg"])
    map_value("body_url", "체중증감", ["wgBudam", "weightDiff", "증감", "체중변화", "diff"])
    map_value("rating_url", "컨디션점수", ["rating", "레이팅", "rt"])
    map_value("odds_url", "현재배당", ["odds", "배당", "winOdds", "dividend", "배당률"])
    map_value("today_odds_url", "현재배당", ["odds", "배당", "winOdds", "dividend", "배당률"])
    map_value("race_record_url", "직전순위", ["ord", "rank", "chaksun", "순위", "착순"])
    map_value("jockey_change_url", "기수", ["jkName", "jockey", "기수", "기수명"])

    # 분석 함수가 빈 값에서 죽지 않게 기본값 보강
    defaults = {
        "거리": 1200, "등급": "", "마령": 4, "부담중량": 55, "초기배당": 12.0, "현재배당": 12.0,
        "직전순위": 5, "지지난순위": 6, "순위": np.nan, "착차": np.nan, "초반위치": 5, "중반위치": 5,
        "4코너위치": 5, "결승위치": 5, "혈통점수": 60, "스피드점수": 60, "지구력점수": 60,
        "거리적성": 60, "컨디션점수": 60, "평균체중": 480, "체중증감": 0, "출전간격일": 21,
        "최근5전": "", "기수승률": 10, "마주승률": 8, "조교사승률": 8, "조교상태": "", "선행가능성": "",
        "마체상태": "", "초반스피드": 60, "막판탄력": 60, "게이트안정": 60, "출발시간": "", "경주명": "19API 실시간"
    }
    ...
    out = ensure(out[COLUMNS])
    return out


def fetch_live_19api_data(course_name, rc_date, race_no):
    statuses = []
    data_map = {}
    for api_key_name, api_label in API_URL_LABELS:
        base_url = api_secret_value(api_key_name)
        df, msg, used = _maru_fetch_one_19api(api_key_name, base_url, str(rc_date).replace("-", ""), int(race_no), course_name)
        data_map[api_key_name] = df
        statuses.append({
            "API": api_label,
            "행수": int(len(df)) if df is not None else 0,
            "컬럼수": int(len(df.columns)) if df is not None and not df.empty else 0,
            "상태": msg,
            "요청URL": _maru_mask_url(used),
        })
    status_df = pd.DataFrame(statuses)
    st.session_state.live_19api_status = status_df
    total_rows = int(status_df["행수"].sum()) if "행수" in status_df.columns else 0
    horse_df = _maru_build_horse_df_from_apis(data_map, course_name, str(rc_date).replace("-", ""), int(race_no))
    if len(horse_df):
        return True, horse_df, f"19API 실시간 성공: 총 {total_rows}행 / 분석 {len(horse_df)}행"
    return False, pd.DataFrame(), f"19API 호출은 했지만 분석 가능한 출전마 데이터 0건 / 총 응답 {total_rows}행"
...
def load_kra_data():
    # 1순위: 형이 원한 19개 공공데이터 API 실제 호출
    if globals().get("live_19api_enabled", True):
        ok19, df19, msg19 = fetch_live_19api_data(course, globals().get("live_rc_date", pd.Timestamp.today().strftime("%Y%m%d")), globals().get("live_race_no", 1))
        if ok19 and len(df19):
            st.session_state.df = df19
            st.session_state.raw_tables = []
            st.session_state.source_status = msg19
            st.session_state.auto_analysis_status = "자동: 19개 공공데이터 API → 실시간 분석 완료"
            return True, msg19
        # 실패해도 화면이 죽지 않게 홈페이지/시트/샘플로 넘어감
        st.session_state.source_status = msg19
        st.session_state.auto_analysis_status = "19API 실패 → 백업 데이터 확인 중"

    # 2순위: 기존 KRA 홈페이지 표 백업
    ...
    if converted:
        ...
        st.session_state.source_status = "KRA 홈페이지 원본표 자동변환 성공"
        st.session_state.auto_analysis_status = "자동: KRA 홈페이지 원본표 → 분석 완료"
        return True, "KRA 홈페이지 성공"
    return False, f"19API/KRA 홈페이지 모두 실패: {st.session_state.get('source_status','')} / 홈페이지: {err}"
...
# 19API 진단표: 실시간이 안 될 때 어떤 API가 실패했는지 바로 확인
if "live_19api_status" in st.session_state and isinstance(st.session_state.live_19api_status, pd.DataFrame) and len(st.session_state.live_19api_status):
    with st.expander("🔌 19개 API 실시간 진단 보기", expanded=False):
        ok_rows = int((st.session_state.live_19api_status["상태"].astype(str) == "OK").sum()) if "상태" in st.session_state.live_19api_status.columns else 0
        total_rows = int(st.session_state.live_19api_status["행수"].sum()) if "행수" in st.session_state.live_19api_status.columns else 0
        st.info(f"성공 API {ok_rows}/19 · 총 수신 {total_rows}행")
        st.dataframe(st.session_state.live_19api_status, use_container_width=True, height=420)

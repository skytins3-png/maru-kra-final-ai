...
def sync_row_to_current_race(row: Dict[str, Any], force_if_stale: bool = True) -> Dict[str, Any]:
    """모바일 안내 카드가 1R 기본값에 머무르지 않게 현재/다음 경주번호와 시간으로 동기화."""
    row = dict(row or {})
    meet = row.get("경마장", "서울") or "서울"
    rc_date = str(row.get("날짜", today_kst()) or today_kst()).replace("-", "")[:8]
    cur = current_live_race_from_schedule(meet, rc_date)
    # 기존 경주시간 보정
    ...
    if not cur:
        return row
    current_no = _safe_race_no(row.get("경주번호", 1))
    current_time = _clean_time_text(row.get("경주시간", ""))
    should_sync = False
    if force_if_stale and current_no != int(cur.get("경주번호", current_no)):
        # 현재 row가 시간없음/결과대기/지난 경주라면 실제 현재 경주로 보정
        m = minutes_until_race(current_time) if current_time else None
        if m is None or m < -5 or cur.get("분전", 999) <= 20:
            should_sync = True
    if should_sync:
        row["경마장"] = cur.get("경마장", meet)
        row["경주번호"] = int(cur.get("경주번호", current_no))
        row["경주시간"] = cur.get("경주시간", current_time)
        row["출발시간"] = cur.get("경주시간", current_time)
        row["시간동기화"] = "Y"
    return row
...
def run_mobile_hub_analysis(meet: str, race_no: int, race_time: str = "", sim_count: int = 1200, risk_mode: str = "균형형") -> Tuple[bool, Dict[str, Any], str]:
    ...
    try:
        ...
        cur_race = current_live_race_from_schedule(meet, rc_date)
...
def mobile_ready_recommendations(limit: int = 20) -> pd.DataFrame:
    ...
    work = hub.copy()
    try:
        # 모바일 화면에서는 실제 현재/다음 경주번호와 시간표를 우선 동기화
...
"""
    ...
    if summary.empty:
        ...
        st.dataframe(summary[show_cols], use_container_width=True, height=260)
    ...
    with st.expander("최근 자동 허브 로그", expanded=False):
        if log.empty:
            ...
            st.dataframe(log[cols].tail(100) if cols else log.tail(100), use_container_width=True, height=360)
...
def render_smart_collection_panel(rc_date: str, meet: str, race_no: int) -> None:
    ...
    st.dataframe(pd.DataFrame(plan_rows), use_container_width=True, height=360)

    c1, c2, c3 = st.columns(3)
    with c1:
        if st.button("🌅 오늘 아침 사전수집", use_container_width=True):
            st.session_state["collection_mode"] = "아침 사전수집"
            st.rerun()
    with c2:
        if st.button("⚡ 경주 직전 실시간만", use_container_width=True):
            st.session_state["collection_mode"] = "실시간 집중"
            st.rerun()
    with c3:
        if st.button("📦 허브만 불러오기", use_container_width=True):
    ...
    if hub.empty:
        ...
        st.dataframe(hub[show_cols] if show_cols else hub, use_container_width=True, height=300)
